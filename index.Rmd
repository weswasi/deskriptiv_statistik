...
institute: "Kriminologiska institutionen, Stockholms universitet"
output:
...
Vad betyder brottslighet och mäter vi brottslighet?

Vad betyder självkontroll och hur mäter vi självkontrol?
...
.pull-left[Kategorier kan rangordnas från hög till låg, mer eller mindre
...
# Två olika typer av numeriska variabler 
...
.pull-left[
```{r, echo=FALSE}
df %>% count(Brottstyp) %>% print(., row.names = FALSE, dims = FALSE, classes = FALSE)
```
]
...
DT::datatable(df_2 %>% 
                select("Person ID", "Ålder"), fillContainer = FALSE, rownames = FALSE, options = list(pageLength = 10, dom = 't'))
```
]

.pull-right[
```{r, echo=FALSE}
df_2 %>% count(Ålder) %>% print(n = 24)
```
]
...
Kan användas för ordinala samt interval och kvotvariabler men __ofta mer lämplig för ordinala variabler__.
]
...
Används __främst för intervall- och kvotvariabler__.
...
# Vilket centralmått spelar roll - Inkapaciteringseffekt?
